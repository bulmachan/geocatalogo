...
#md-clickdef						{font-family: Arial,sans-serif; font-size: 9pt; color: #0000FF; text-align: center; cursor: pointer; text-align: center; padding: 0px; margin-left: 0px; margin-left: 0px; margin-top: 5px; margin-bottom: 5px;}
#md-menu							{font-family: Arial,sans-serif; font-size: 9pt; color: #0000FF; text-align: center; cursor: pointer; text-align: center; padding: 0px; margin-left: 0px; margin-left: 0px; margin-top: 5px; margin-bottom: 5px;}
...
#md-thumbnail-desc					{margin-top: 5px; float: left;}
...
<xsl:template name="header">
	...
	<!-- Build subtitle -->
	<div class="md-subtitle">FGDC<xsl:text/>
...
<xsl:template name="thumbnail">
	<div class="md-color">
		<div class="md-master">
			<div class="md-mastertitle">Binary Enclosures</div>
			<div style="background-color: #FFFFCC;">
			<img id="md-thumbnail">
			<xsl:attribute name="SRC">getThumbnail.jsp?w=150&amp;h=100&amp;layer=<xsl:value-of select="/metadata/idinfo/citation/citeinfo/ftname[normalize-space(.)]"/></xsl:attribute>
			</img>
			<div id="md-thumbnail-desc"><b>Thumbnail:</b><br/>
				<b>Enclosure type:</b> Picture</div><div style="clear:both"></div>
			</div>
		</div>
...
<xsl:template name="description">
	<xsl:if test="/metadata/idinfo[normalize-space(.) != '']">
		<div id="md-description" class="md-color">
			<div class="md-master">
				<div class="md-mastertitle">Description</div>
			</div>
					...
					<div class="md-itemhide">
						...
						<table class="md-grid">
							...
							<xsl:for-each select="westbc[normalize-space(.) != '']">
								<tr><td>West</td><td><xsl:value-of select="."/> (latitude)</td></tr>
							</xsl:for-each>
							<xsl:for-each select="eastbc[normalize-space(.) != '']">
								<tr><td>East</td><td><xsl:value-of select="."/> (latitude)</td></tr>
							</xsl:for-each>
							<xsl:for-each select="northbc[normalize-space(.) != '']">
								<tr><td>North</td><td><xsl:value-of select="."/> (longitude)</td></tr>
							</xsl:for-each>
							<xsl:for-each select="southbc[normalize-space(.) != '']">
								<tr><td>South</td><td><xsl:value-of select="."/> (longitude)</td></tr>
							</xsl:for-each>							
						</table>
					</div>
				</xsl:for-each>				
...
<xsl:template name="metadata_fgdc_plus_stylesheet">
	...
	<div class="md-detailhelp">
		...
		<div class="md"><b>Version: </b>2.0 </div>
...
<xsl:template match="albers | azimequi | equicon | equirect | gnomonic | gvnsp | lamberta |      lambertc | mercator | miller | modsak | obqmerc | orthogr | polarst | polycon | robinson |      sinusoid | spaceobq | stereo | transmer | vdgrin">
  <xsl:apply-templates select="*"/>
</xsl:template>

<!-- Projections defined in the 8.0 ESRI Profile -->
<xsl:template match="behrmann | bonne | cassini | eckert1 | eckert2 | eckert3 | eckert4 |      eckert5 | eckert6 | gallster | loximuth | mollweid | quartic | winkel1 | winkel2">
  <xsl:apply-templates select="*"/>
</xsl:template>
		...
		<xsl:if test="address[normalize-space(.) != ''] or city[normalize-space(.) != ''] or state[normalize-space(.) != ''] or postal[normalize-space(.) != ''] or country[normalize-space(.) != '']">
			<div class="md-indent">
				...
				<xsl:for-each select="country[normalize-space(.) != '']">
					<div class="md"><b>County: </b><xsl:value-of select="."/></div>
				</xsl:for-each>
			</div>
		</xsl:if>
	</xsl:for-each>
